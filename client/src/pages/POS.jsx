...
const handlePrintInvoice = () => {
  ...
  const processSale = async (installments = []) => {
    ...
    try {
      ...
      // 🔥 Refrescar solo los datos de productos y gas (no toda la página)
      setTimeout(() => {
        fetchProducts(); // Refrescar productos
        fetchGasTypes(); // Refrescar tipos de gas
        fetchWashingMachines(); // Refrescar lavadoras
      }, 500); // Esperar 0.5 segundos para que se muestre el toast
    } catch (error) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl">
            ...
            <div className="p-4 space-y-3">
              <button
                className="w-full h-20 rounded-xl bg-blue-600 text-white text-lg font-bold hover:bg-blue-700 transition-colors"
                onClick={() => {
              ...
              >
                <div className="text-center">
                  <div className="text-lg font-bold">🔄 INTERCAMBIO</div>
                  <div className="text-xs opacity-90">Cliente entrega cilindro vacío</div>
                  <div className="text-sm mt-1">Precio: ${Number(gasModal.item?.precio_base || 0).toLocaleString('es-EC')}</div>
                </div>
              </button>
              
              <button
                className="w-full h-20 rounded-xl bg-amber-500 text-white text-lg font-bold hover:bg-amber-600 transition-colors"
                onClick={() => {
                  const it = gasModal.item;
                  const precio =
              ...
              >
                <div className="text-center">
                  <div className="text-lg font-bold">🛒 COMPRA COMPLETA</div>
                  <div className="text-xs opacity-90">Cliente NO entrega cilindro (cobra depósito)</div>
                  <div className="text-sm mt-1">Precio: ${(Number(gasModal.item?.precio_base || 0) + Number(gasModal.item?.precio_envase || 0)).toLocaleString('es-EC')}</div>
                </div>
              </button>
              
              <button
                className="w-full h-12 rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors font-medium"
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                <div className="text-xs text-amber-800 font-medium mb-1">💡 Información importante:</div>
                <div className="text-xs text-amber-700 space-y-1">
                  <div>• <strong>Intercambio:</strong> Cliente entrega cilindro vacío y solo paga por el gas</div>
                  <div>• <strong>Compra completa:</strong> Cliente compra cilindro nuevo (gas + envase)</div>
                  <div>• <strong>Depósito del envase:</strong> ${Number(gasModal.item?.precio_envase || 0).toLocaleString('es-EC')}</div>
                </div>
              </div>
            </div>
          </div>
