    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const it of items) {
        ...
        if (it.productId) {
          ...
          // 🔥 VALIDACIÓN: Solo verificar stock de gas lleno
          // No validar stock_vacios porque aumenta cuando cliente entrega cilindro vacío
...
router.delete("/users/:id", auth, async (req, res) => {
  try {
    ...
    const [salesCount, rentalsCount] = await Promise.all([
      prisma.sale.count({ where: { userId: userId } }),
      prisma.rental.count({ where: { userId: userId } }),
    ]);

    if (salesCount > 0 || rentalsCount > 0) {
      return res.status(400).json({
        error: "No se puede eliminar usuario con ventas o alquileres asociados",
        details: {
          salesCount,
          rentalsCount,
        },
  ...
  try {
    ...
    const formattedSales = filteredSales.map((sale) => {
      ...
      return {
        ...
        total: Number(sale.items.reduce((sum, item) => sum + (Number(item.subtotal) || 0), 0)).toFixed(2),
...
async function computeSummaryForDateRange(startDate, endDate) {
  try {
    ...
    (sum, sale) => sum + sale.items.reduce((itemSum, item) => itemSum + (Number(item.subtotal) || 0), 0),
      0
    );
    ...
    const totalesPorMetodo = sales.reduce((acc, sale) => {
      ...
      const saleTotal = sale.items.reduce((itemSum, item) => itemSum + (Number(item.subtotal) || 0), 0);
acc[metodo] = (acc[metodo] || 0) + saleTotal;
      return acc;
    }, {});
...
router.post("/rentals", auth, async (req, res) => {
  try {
    ...
    if (typeof scheduledReturnDate === 'string') {
      // Para alquiler por hora, usar parsing original
      if (rentalType !== 'OVERNIGHT') {
        ...
        // Para alquiler por amanecida, forzar timezone Colombia
        if (scheduledReturnDate.includes('T')) {
      ...
      const rental = await tx.rental.create({
        data: {
          ...
          ...(overnightAdditionalPrice && {
            overnightAdditionalPrice: String(overnightAdditionalPrice),
          }),
